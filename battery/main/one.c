...
#define VOLTAGE_DIVIDER_FACTOR 3.0f // If a divider is used, set factor = (R1+R2)/R2
#define V_EMPTY_MV 3000             // Battery empty voltage (mV)
#define V_FULL_MV 4200              // Battery full voltage (mV)
...
static void read_battery(float *voltage_v, int *adc_raw_avg, int samples)
{
  ...
  *adc_raw_avg = avg_raw;
  ...
  if (do_calibration)
  {
    ...
    float bat_v = (voltage_mv * VOLTAGE_DIVIDER_FACTOR) / 1000.0f;
    *voltage_v = bat_v;
  }
  else
  {
    // Fallback: estimate assuming 12-bit ADC and 3.3V reference
    float v = (avg_raw * (3.3f / 4095.0f)) * VOLTAGE_DIVIDER_FACTOR;
    *voltage_v = v;
  }
}
...
static void display_init(void)
{
  ...
  spi_bus_config_t buscfg = {
      ...
      .quadwp_io_num = -1,
      .quadhd_io_num = -1,
      .max_transfer_sz = EXAMPLE_LCD_H_RES * 40 * sizeof(lv_color_t),
  };
  ...
  // draw buffer
  disp_buf1 = heap_caps_malloc(EXAMPLE_LCD_H_RES * 40 * sizeof(lv_color_t), MALLOC_CAP_DMA);
  assert(disp_buf1 != NULL);
  lv_disp_draw_buf_init(&draw_buf, disp_buf1, NULL, EXAMPLE_LCD_H_RES * 40);
...
static void display_update(float voltage, int adc_raw, int pct_milli)
{
  ...
  if (label_voltage)
  {
    ...
    ESP_LOGI(TAG, "display_update: %s", buf);
...
void app_main(void)
{
  ...
  while (1)
  {
    float voltage = 0.0f;
    int adc_raw = 0;
    read_battery(&voltage, &adc_raw, ADC_SAMPLES);
